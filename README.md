...
1. 행위
    1. 3자리 난수를 생성한다
        *[x] 난수는 중복되지 않아야 한다.
        *[x] 난수는 1~9까지의 숫자여야 한다.
    2. 유저로부터 3자리 수를 입력받는다.
        *[x] 유저의 수는 string으로 되어있다.
        *[x] 입력된 수는 중복되지 않아야 한다.
        *[x] 수는 1~9까지의 숫자여야 한다.
    3. 두개의 숫자리스트를 비교한다.
        *[x] 동일한 숫자가 있는지 비교한다.
        *[x] 동일한 위치의, 동일한 숫자가 있는지 비교한다.
    4. 결과를 반환한다.
        *[x] 숫자가 동일하기만 하면 볼을 반환한다.
        *[x] 숫자가 위치까지 동일하면 스트라이크를 반환한다.
        *[x] 세숫자중 어느 숫자도 일치하지 않으면 낫싱을 반환한다.
    5. 과정을 반복한다.
        *[ ] strike가 3개가 될때까지 과정을 반복한다.
        *[ ] strike가 3개가 되면 게임이 종료 된다.
    6. 게임 재시작 여부를 결정한다.
        *[ ] 게임이 끝난 후 1을 입력하면 재시작, 2를 입력하면 프로그램 종료
2.도메인
    1. 수 : 1~9까지인지 체크
    2. 수들 : 중복 체크, 3자리 체크
    3. 수 생성기: 중복되지 않은 3자리 수 생성
    4. 컴퓨터: 3자리 수, 유저와 수 비교
